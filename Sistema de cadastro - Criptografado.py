...
class Usuario:
    ...
    def atualizar_senha(self,nova_senha):
        self.senha=bcrypt.hashpw(nova_senha.enconde('utf-8'),bcrypt.gensalt())
usuarios=[]
...
def atualizar_senha():
    ...
    for usuario in usuarios:
        if usuario.email == email:
            ...
            if bcrypt.chekpw(senha_antiga.encode('utf-8'),usuario.senha):
...
def exibir_senha():
    ...
    senha=input('Digite sua senha: ')
    for usuario in usuarios:
        if usuario.email == email and bcrypt.checkpw(senha.encode('utf-8'),usuario.senha):
            data={
                'Email':[usuario.email],
                'senha':[usuario.senha]
            }
